...
列表是由一系列特定顺序排列的元素组成。你可以创建包含字母表的字母、数字0~9或所有家庭成员姓名的列表；也可以将任何东西加入列表中，其中的元素可以没有任何关系。鉴于列表通常包含多个元素，给列表指定一个表示复数的名称（如letters、digits或names）是一个不错的主意。
...
例如，下面的代码将从列表bicycles中提取出第一款自行车：
...
运行这个程序，会输出以下结果：
	...
	------------------------个性化消息------------------------
	Hello Hardy, would you like to learn some Python today?
	...
	--------------------将字符串转换为数字--------------------
	原来strNum的类型为： <class 'str'>
	转换后strNum的类型为： <class 'int'>

### 2.7 题外话

当初计划是在本月21号完成基础篇，如今看来，怕是无法如愿了。上个月实在是太忙了，赶公司的项目赶到作息紊乱、睡眠不足了，这几天才慢慢缓过来。回过头来看，项目确实比较赶，但关键还是我不太会安排时间，而且做事的时候容易分心，一件事情没做好，又会被另一件事情所打扰，自己还是太嫩了。

转眼间8月份都过去一半了，下半年还有许多想做的事情，尽快找到合适的节奏，提高学习效率吧。
