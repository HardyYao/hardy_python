...
class getZhengZhouSum(object):
    def get_data(self, place_zhengzhou, variety, contract_id, year, month, day, path):
        req = requests.get(
            'http://www.czce.com.cn/portal/DFSStaticFiles/Future/{}/{}{}{}/FutureDataHolding.htm'.format(year, year,
                                                                                                         month, day),
            headers=headers())
        if str(req) != '<Response [200]>':
            ...
            if len(tab) > 0:
                ...
                for trItem in tr:
                    contract_index += 1
                    if trItem.text.replace('\n', '').replace('\xa0', '').find('品种：{}'.format(variety)) == True:
                        break
