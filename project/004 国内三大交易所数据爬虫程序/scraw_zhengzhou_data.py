...
class getZhengZhouFutures(object):
    def get_data(self, place_zhengzhou, variety, contract_id, year, month, day, path):
        req = requests.get('http://www.czce.com.cn/portal/DFSStaticFiles/Future/{}/{}{}{}/FutureDataHolding.htm'.format(year, year, month, day), headers=headers())
        ...
        if str(req) != '<Response [200]>':
            ...
            if len(tab) > 0:
                ...
                for trItem in tr:
                    contract_index += 1
                    if trItem.text.replace('\n', '').replace('\xa0', '').startswith('合约：{}'.format(contract_id)) == True:
                        break
                ...
                if contract_index < len(tr):
                    ...
                    for trIter in tr[contract_index+1:]:
                        ...
                        if tempList[0:9]:
                            ...
                            worksheet.write(int(tempList[0]), 1, place_zhengzhou)
